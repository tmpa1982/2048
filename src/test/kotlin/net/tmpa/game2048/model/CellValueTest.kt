...
class CellValueTest {
    ...
    fun `test enum values`() {
        ...
        for (cellValue in CellValue.entries.filter { it.hasNext }) {
    ...
    fun `next value on 2048 throws exception`() {
        noNextState(CellValue.V2048)
    }

    @Test
    fun `next value on obstacle throws exception`() {
        noNextState(CellValue.OBSTACLE)
    }

    private fun noNextState(cellValue: CellValue) {
        assertFalse(cellValue.hasNext)
        assertThrows<IllegalStateException> { cellValue.getNext() }
    }
}
